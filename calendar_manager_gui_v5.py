...
class Month_handler:
    ...
    def generate_ics(self):
        ...
        def vevent_timed(d: date, start_s: str, end_s: str, summary: str) -> str:
            ...
            start_local = datetime(d.year, d.month, d.day, h1, m1).astimezone(TZ)
            # end_local   = datetime(d.year, d.month, d.day, h2, m2, tzinfo=TZ)
            end_local   = datetime(d.year, d.month, d.day, h2, m2).astimezone(TZ)
            ...
            return "\r\n".join([
                ...
                f"DTSTART;TZID=Europe/Prague:{start_local.strftime('%Y%m%dT%H%M%S')}",
                # f"DTEND:{fmt_dt_utc(end_local)}",
                f"DTEND;TZID=Europe/Prague:{end_local.strftime('%Y%m%dT%H%M%S')}",
